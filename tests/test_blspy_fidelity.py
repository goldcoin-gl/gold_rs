...
# make sure gold_rs counterpart behaves the same as blspy
def test_bls() -> None:
    print()
    for round in range(200):
        ...
        sig2 = gold_rs.AugSchemeMPL.sign(sk2, msg)
        assert bytes(sig1) == bytes(sig2)

        ####  verify()  ####
        assert blspy.AugSchemeMPL.verify(pk1, msg, sig1) == True
        assert gold_rs.AugSchemeMPL.verify(pk2, msg, sig2) == True

        ####  sign() (custom augment)  ####
        pk11 = blspy.AugSchemeMPL.derive_child_pk_unhardened(pk1, 1)
        pk21 = gold_rs.AugSchemeMPL.derive_child_pk_unhardened(pk2, 1)
        assert bytes(pk11) == bytes(pk21)
        sig1 = blspy.AugSchemeMPL.sign(sk1, msg, pk11)
        sig2 = gold_rs.AugSchemeMPL.sign(sk2, msg, pk21)
        assert bytes(sig1) == bytes(sig2)
        ...
        for idx in range(1000, 1020):
            ...
            pk21 = gold_rs.AugSchemeMPL.derive_child_pk_unhardened(pk2, idx)
            assert bytes(pk11) == bytes(pk21)

            sk11 = blspy.AugSchemeMPL.derive_child_sk_unhardened(sk1, idx)
            sk21 = gold_rs.AugSchemeMPL.derive_child_sk_unhardened(sk2, idx)
        ...
        for idx in range(100, 120):
            ...
            sk21 = gold_rs.AugSchemeMPL.derive_child_sk(sk2, idx)
            assert bytes(sk11) == bytes(sk21)

        ####  g2_from_message()  ####
        msg = randbytes(100)
        sig1 = blspy.AugSchemeMPL.g2_from_message(msg)
        sig2 = gold_rs.AugSchemeMPL.g2_from_message(msg)
        assert bytes(sig1) == bytes(sig2)
        ...
        for _ in range(10):
            ...
            sigs2.append(gold_rs.AugSchemeMPL.g2_from_message(msg))

        aggsig1 = blspy.AugSchemeMPL.aggregate(sigs1)
        aggsig2 = gold_rs.AugSchemeMPL.aggregate(sigs2)
        ...
        # size constants
        assert blspy.GTElement.SIZE == gold_rs.GTElement.SIZE
        assert blspy.G1Element.SIZE == gold_rs.G1Element.SIZE
        assert blspy.G2Element.SIZE == gold_rs.G2Element.SIZE
        assert blspy.PrivateKey.PRIVATE_KEY_SIZE == gold_rs.PrivateKey.PRIVATE_KEY_SIZE
        ...
        pair22 = gold_rs.GTElement.from_bytes(bytes(pair2))
...
# ------------------------------------- 8< ----------------------------------
#
# this is the original test from blspy, but converted to use gold_rs instead
